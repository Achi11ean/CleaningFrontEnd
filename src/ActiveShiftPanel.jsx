...
// Consultation stays available by default, preserving the original staff panel.
// Pass hasConsultation={false} when absence is confirmed, or use the resolver.
...
export default function ActiveShiftPanel({ refreshKey, onShiftUpdated, hasConsultation = true, resolveHasConsultation, debug = false }) {
  ...
  const log = useCallback((event, details) => {
    if (debug) console.log(`[StaffTasks] ${event}`, details);
  }, [debug]);
  ...
  const staffId = activeShift?.shift?.staff_id;
  ...
  const loadActiveShift = useCallback(async () => {
    ...
    try {
      const { data } = await authAxios.get("/staff/shifts/active");
  ...
  const loadTasks = useCallback(async () => {
    ...
    try {
      ...
      if (templates.length === 0 && open.length === 0) {
        setListDiagnostic(`No active checklists are available for client #${clientId}. Ask an administrator to verify the client and activate the intended task list.`);
      }
    } catch (error) {
  ...
  const startChecklist = async () => {
    if (actionLock.current || tasksLoading || !listId || !clientId || !staffId) {
    ...
    try {
      ...
      const result = existing || (await authAxios.post(`/cleaning/task-lists/${listId}/start`, scheduleId == null ? {staff_id:staffId} : {schedule_id:scheduleId,staff_id:staffId})).data.cleaning_session;
  ...
  const toggleTask = async task => {
    if (actionLock.current || tasksLoading || !staffId || !session || session.status !== "in_progress") return;
    actionLock.current = true; setBusyTask(task.id); setTaskError("");
    const currentSessionId = session.id;
    const currentShiftId = shiftId;
    taskRequest.current++;
    try {
      // Attribute this task to the staff owner returned by the active-shift route.
      const {data} = await authAxios.patch(`/cleaning/tasks/${task.id}`, {is_completed:!task.is_completed,staff_id:staffId});
  ...
  const completeChecklist = async () => {
    if (actionLock.current || tasksLoading || loading || shiftError || !staffId || !session || session.status !== "in_progress") return;
    ...
    try {
      // The server validates current shared progress before closing.
      // Attribute completion to the staff owner of this shift.
      const {data} = await authAxios.post(`/cleaning/sessions/${currentSessionId}/finalize`, {staff_id:staffId});
  ...
  const checkOut = async () => {
    ...
    try {
      await authAxios.post("/staff/shifts/check-out",{image_urls:photoUrl?[photoUrl]:[],message:message.trim()||null});
  ...
  return <CleaningTheme className="asp-theme"><style>{styles}</style><section className="asp-panel" aria-label="Active staff cleaning shift">
    <header className="asp-header"><span className="asp-mark"><CleaningSparkle/></span><div><p className="asp-eyebrow">Staff cleaning workspace</p><h3>{activeShift ? "Your active shift" : "Shift status"}</h3></div>{activeShift && <span className="asp-live">Checked in</span>}</header>
    ...
    {activeShift && <>
      ...
      {view==="consultation" && consultationAvailable && <div className="asp-consultation"><ConsultationChecklist key={`${clientId}-${consultationVersion}`} clientId={clientId} label="Shift Checklist"/></div>}
      {view==="tasks" && <div className="asp-tasks">
        ...
        {!staffId && <p className="asp-error" role="alert">Staff identity is missing from this shift. Reload the shift before changing tasks.</p>}
        ...
        {!session && selectedList && <div className="asp-start"><h4>{selectedList.name}</h4>{selectedList.description && <p>{selectedList.description}</p>}<p>{(selectedList.items||[]).filter(item=>item.is_active).length} tasks ready for this cleaning.</p><button type="button" className="asp-primary" disabled={busy || tasksLoading || loading || Boolean(shiftError) || !staffId} onClick={startChecklist}>{starting ? "Opening checklist…" : "Start / join checklist"}</button></div>}
        {session && <>
          ...
          {session.status === "in_progress" && <div className="asp-complete">
            ...
            <button type="button" className="asp-primary" disabled={busy || tasksLoading || loading || Boolean(shiftError) || !staffId} onClick={completeChecklist}>{finalizing ? "Completing…" : "✓ Complete"}</button>
          </div>}
        </>}
      </div>}
