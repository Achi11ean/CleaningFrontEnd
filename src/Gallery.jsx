...
const Gallery = () => {
  ...
  /* ---------------------------- Render -------------------------- */
  return (
    <CleaningTheme className="gallery-page-theme" showLoader={false}>
    <div className="gl-page relative min-h-screen antialiased selection:bg-cyan-400/30">
      <style>{`
        .gallery-page-theme .gl-page { position:relative; min-width:0; }
        .gallery-page-theme .gl-muted { color:var(--ct-muted, #a9c0d3); }
        .gallery-page-theme .gl-photo-card { padding:0; border-color:var(--ct-line, #7dd3fc26); }
        .gallery-page-theme .gl-section-label { color:var(--ct-teal, #8edcea); }
        .gallery-page-theme .gl-intro-badge { display:inline-flex; align-items:center; gap:9px; }
        .gallery-page-theme .gl-intro-badge svg { width:14px; height:14px; }
        .gallery-page-theme .gl-cta { min-height:44px; text-align:center; justify-content:center; }
      ...
      <header className="relative px-6 pb-14 pt-32 md:pb-16 md:pt-40">
        <div {...reveal()} className="mx-auto max-w-3xl text-center">
          <p className="ct-eyebrow gl-intro-badge mb-6 rounded-full border border-sky-400/25 bg-white/[0.04] px-5 py-2 backdrop-blur-md">
            <CleaningSparkle />
            Our work
          </p>
          ...
          <p className="gl-muted mx-auto mt-6 max-w-xl text-base leading-relaxed md:text-lg">
      ...
      <main className="relative mx-auto max-w-7xl px-6 pb-28">
        <div {...reveal(150)} className="mb-10 flex items-center gap-5">
          <h2 className="ct-eyebrow gl-section-label whitespace-nowrap">
            {loading || error
              ? "The collection"
        ...
        {!loading && error && (
          <div className="flex flex-col items-start gap-4 rounded-3xl border border-rose-400/25 bg-rose-400/[0.07] p-8 backdrop-blur-md sm:flex-row sm:items-center sm:justify-between">
            ...
            <Link to="/contact" className="ct-btn gl-cta">Ask us directly</Link>
          </div>
        )}
        {/* Empty */}
        {!loading && !error && photos.length === 0 && (
          <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-12 text-center backdrop-blur-xl">
            ...
            <Link to="/contact" className="ct-btn gl-cta mt-7">Book a clean</Link>
          </div>
        )}
        {/* Masonry grid */}
        {!loading && !error && photos.length > 0 && (
          <div className="gl-masonry">
            {photos.map((photo, i) => (
              ...
              type="button"
                ...
                className={`ct-card gl-photo-card gl-tile group relative block overflow-hidden rounded-3xl border text-left transition-all duration-500 hover:-translate-y-1.5 hover:border-sky-300/30 hover:shadow-xl hover:shadow-sky-500/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300 ${
                  shown ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
                }`}
              >
                <img
          ...
          >
            <div className="relative flex flex-col lg:flex-row">
              ...
              <div className="flex flex-col justify-center border-t border-white/10 p-8 lg:w-1/3 lg:border-l lg:border-t-0 lg:p-10">
                ...
                <Link to="/contact" className="ct-btn gl-cta mt-8 w-full">Book a clean like this</Link>
              </div>
            </div>
          </div>
        </div>
