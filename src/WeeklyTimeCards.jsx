...
export default function WeeklyTimeCards({ staffList, weekStart }) {
  ...
  const toggleStaff = async (staffId) => {
    ...
    // ❌ Managers cannot open their own timecards
    if (role === "manager" && staff?.id === staffId) return;
  ...
  return (
    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
      {staffList.map((row) => {
        const isSelf = staff?.id === row.staff_id && role === "manager";
        ...
        return (
          ...
          >
            <div
              ...
              onClick={() => !isSelf && toggleStaff(row.staff_id)}
            >
                  ...
                  entryMap[row.staff_id].map((entry) => (
                    <EditTimeCard
                      ...
                      onDelete={(deletedId) => {
                        setEntryMap((prev) => ({
                          ...prev,
                          [row.staff_id]: prev[row.staff_id].filter(
                            (e) => e.id !== deletedId
                          ),
                        }));
                      }}
