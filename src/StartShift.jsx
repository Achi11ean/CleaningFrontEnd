...
function StartShiftFlow({ schedule, onStarted }) {
  ...
  const confirmedShift = useRef(null);
  ...
  const notifyParent = async () => {
    ...
    setCallbackError("");
    try {
      await onStarted?.(confirmedShift.current);
  ...
  const start = async () => {
    ...
    if (schedule.client_id != null && !sameId(schedule.client_id,clientId)) {
      setError("The schedule and client do not match. Refresh the booking before checking in."); return;
    }
    lock.current = true;
    ...
    try {
      ...
      const current = (await authAxios.get("/staff/shifts/active")).data;
      ...
      confirmedShift.current = current.active ? current.shift : null;
      if (!current.active) {
        setCheckedIn(false);
        setPhase("location");
        let position;
        ...
        setPhase("checkin");
        try {
          const result = await authAxios.post("/staff/shifts/check-in", {
            client_id: clientId, schedule_id: scheduleId,
            lat: position.coords.latitude, lng: position.coords.longitude,
          });
          confirmedShift.current = result.data.shift;
        } catch (checkInError) {
          if (!mounted.current) return;
          ...
          let recovered = false;
          try {
            const verified = (await authAxios.get("/staff/shifts/active")).data;
            recovered = isThisShift(verified);
            if (recovered) confirmedShift.current = verified.shift;
          }
      ...
      // Use the shift's actual staff owner, not a profile ID or client ID.
      if (!confirmedShift.current?.staff_id || !confirmedShift.current?.id) {
        const verified = (await authAxios.get("/staff/shifts/active")).data;
        if (!mounted.current) return;
        if (!isThisShift(verified)) {
          setCheckedIn(false);
          setError("The active shift changed. Refresh the booking before continuing."); return;
        }
        confirmedShift.current = verified.shift;
      }
      const shift = confirmedShift.current;
      if (!shift?.staff_id || !shift?.id || !sameId(shift.client_id,clientId) || !sameId(shift.schedule_id,scheduleId)) {
        setError("Check-in is saved, but the server did not return a matching shift with staff identity. Retry setup to verify it."); return;
      }
      ...
      for (let index=0; index<activeLists.length; index++) {
        ...
        try {
          ...
          // Attribute newly started sessions to the staff owner of this shift.
          const result = await authAxios.post(`/cleaning/task-lists/${list.id}/start`, {schedule_id:scheduleId,staff_id:staffId});
  ...
  return <CleaningTheme className="ass-theme"><section className="ass-panel" aria-label="Start staff cleaning shift"><style>{styles}</style>
    <div className="ass-heading"><span aria-hidden="true">✦</span><div><p>Staff cleaning</p><h4>{ready ? "Ready to clean" : checkedIn ? "Finish checklist setup" : "Start this shift"}</h4></div></div>
