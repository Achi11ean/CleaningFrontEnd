...
const ReviewList = () => {
  ...
  /* ---------------------------- Render -------------------------- */
  return (
    <CleaningTheme className="reviews-page-theme" showLoader={false}>
    <div className="rv-page relative min-h-screen antialiased selection:bg-cyan-400/30">
      <style>{`
        .reviews-page-theme .rv-page { position:relative; min-width:0; }
        .reviews-page-theme .rv-muted { color:var(--ct-muted, #a9c0d3); }
        .reviews-page-theme .rv-review-card { margin:0; padding:1.75rem; min-width:0; border-color:var(--ct-line, #7dd3fc26); }
        .reviews-page-theme .rv-section-label { color:var(--ct-teal, #8edcea); }
        .reviews-page-theme .rv-intro-badge { display:inline-flex; align-items:center; gap:9px; }
        .reviews-page-theme .rv-intro-badge svg { width:14px; height:14px; }
        .reviews-page-theme .rv-cta { min-height:44px; text-align:center; justify-content:center; }
      ...
      <header className="relative px-6 pb-14 pt-32 md:pb-16 md:pt-40">
        <div {...reveal()} className="mx-auto max-w-3xl text-center">
          <p className="ct-eyebrow rv-intro-badge mb-6 rounded-full border border-sky-400/25 bg-white/[0.04] px-5 py-2 backdrop-blur-md">
            <CleaningSparkle />
            In their words
          </p>
          ...
          <p className="rv-muted mx-auto mt-6 max-w-xl text-base leading-relaxed md:text-lg">
        ...
        <div {...reveal(220)} className="mt-10 flex justify-center">
          <button
            ...
            className="ct-btn rv-cta rv-shine"
          >
            Write a review
          </button>
        </div>
      ...
      <main ref={listRef} className="relative mx-auto max-w-6xl scroll-mt-28 px-6 pb-28">
        <div {...reveal(280)} className="mb-10 flex items-center gap-5">
          <h2 className="ct-eyebrow rv-section-label whitespace-nowrap">
            {loading || error
              ? "The reviews"
        ...
        {!loading && error && (
          <div className="flex flex-col items-start gap-4 rounded-3xl border border-rose-400/25 bg-rose-400/[0.07] p-8 backdrop-blur-md sm:flex-row sm:items-center sm:justify-between">
            ...
            <Link to="/contact" className="ct-btn rv-cta">Ask us directly</Link>
          </div>
        )}
        {/* Empty */}
        {!loading && !error && reviews.length === 0 && (
          <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-12 text-center backdrop-blur-xl">
            ...
            <button
              type="button"
              onClick={() => setShowCreate(true)}
              className="ct-btn rv-cta rv-shine mt-7"
            >
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {currentReviews.map((review, i) => (
              <figure
                ...
                className={`ct-card rv-review-card group flex flex-col rounded-3xl border backdrop-blur-xl transition-all duration-500 hover:-translate-y-1.5 hover:border-sky-300/30 hover:bg-white/[0.06] hover:shadow-xl hover:shadow-sky-500/10 ${
                  shown ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
                }`}
              >
                {/* Attribution */}
                ...
                {/* The words */}
                <blockquote className="rv-scroll rv-muted max-h-52 flex-1 overflow-y-auto pr-1 text-sm leading-relaxed">
